...
function SingleAccountLink({ icon, title, link, showLeftPanelHandler }) {
    ...
    return (
        <Link onClick={() => showLeftPanelHandler()} to={link} className={`flex items-center pl-1 py-3 ${pathname === link ? 'bg-zinc-800' : ''} hover:bg-zinc-800 rounded-md max-md:active:bg-zinc-700`}>
            <FontAwesomeIcon
                icon={icon}
